...
class GameState():
    ...
    def generate_faller(self):
        self.gem_list = ['Y', 'G', 'B', 'R', 'P', 'T', 'W']
        faller_list = [random.choice(self.gem_list) for i in range(3)]
        return faller_list

    # def create_faller_list(self, inpt):
    def create_faller_list(self):
        faller_col = str(random.choice(list(range(self.cols))))
        faller_list = self.generate_faller()
